            ...
            try:
                ...
                for i, process in enumerate(self.processes):
                    if process.poll() is not None:
                        ...
                        print(
                            f"❌ {agent_name} process has died (exit code: {process.poll()})"
                        )
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    status_response = await client.get(
                        f"http://localhost:8000/workflow/{workflow_id}"
                    )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   Recent workflows (24h): {metrics.get('recent_workflows_24h', 0)}"
                    )
                    print(
                        f"   Success rate (24h): {metrics.get('success_rate_24h', 0):.1%}"
                    )
